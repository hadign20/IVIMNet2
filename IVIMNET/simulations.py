...
def sim(SNR, arg):
    ...
    if arg.fit.model == 'bi-exp':
        ...
        IVIM_signal_noisy, D, f, Dp, f2, Dp2 = sim_signal(SNR, arg.sim.bvalues, sims=arg.sim.sims,
        ...
        if arg.sim.repeats > 1:
            ...
            for aa in range(arg.sim.repeats):
                ...
                if arg.fit.model == 'bi-exp':
                    ...
                    matNN[aa] = print_errors(np.squeeze(D), np.squeeze(f), np.squeeze(Dp), paramsNN[aa], np.squeeze(f2),
                                             np.squeeze(Dp2))
            ...
            if arg.fit.model == 'bi-exp':
                ...
                matNN = print_errors(np.squeeze(D), np.squeeze(f), np.squeeze(Dp), paramsNN, np.squeeze(f2),
                                     np.squeeze(Dp2))
    ...
    if arg.fit.do_fit:
        ...
        if arg.fit.model == 'bi-exp':
            ...
            matlsq = print_errors(np.squeeze(D), np.squeeze(f), np.squeeze(Dp), paramsf, np.squeeze(f2),
                                  np.squeeze(Dp2))
...
def sim_signal(SNR, bvalues, sims=100000, Dmin=0.5 / 1000, Dmax=5.0 / 1000, fmin=0.0, fmax=0.7, Dsmin=0.05, Dsmax=0.3,
               fp2min=0, fp2max=0.3, Ds2min=0.2, Ds2max=0.5,
    ...
    for aa in range(len(D)):
        if bi_exp:
            ...
            data_sim[aa, :] = fit.tri_exp(bvalues, 1 - f[aa][0] - f2[aa][0], D[aa][0], f[aa][0], Dp[aa][0], f2[aa][0],
                                          Dp2[aa][0])
